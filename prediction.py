...
def prediction():
    ...
    model_body, model = create_model(anchors, class_names, regions, load_pretrained=False, freeze_body=False)
    model_body.load_weights(weights2load)
    ...
    for fn in fns:
        image_id = fn.split('.')[0]
        img = Image.open(image_path+fn)
        processed_images = [img.resize((n_strips_x, n_strips_y), Image.BICUBIC)]
        processed_images = [np.array(image, dtype=np.float) for image in processed_images]
        processed_images = [image / 255. for image in processed_images]
        processed_images = np.array(processed_images)
        image_data = np.array([np.expand_dims(image, axis=0) for image in processed_images])
        yolo_ouput = yolo_head(model_body.output, anchors, len(class_names))
        input_image_shape = K.placeholder(shape=(2,))
        boxes, scores, classes = yolo_eval(yolo_ouput, input_image_shape, score_threshold=0.07, iou_threshold=0.0)
        sess = K.get_session()
